...
def sgw_gpu_np(xs,  xt,  nproj=200,  P=None):
    """Numpy version of sliced gromov wasserstein
    """
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    return sgw_gpu(torch.Tensor(xs), torch.Tensor(xt), device=device, nproj=nproj, P=P).numpy()
...
def sink_(xs, xt, device, nproj=200, P=None):  # Delta operator (here just padding)
    ...
    if dim_d < dim_p:
        ...
        xs2 = torch.cat(
            (xs, torch.zeros((xs.shape[0], dim_p-dim_d)).to(device)), dim=1)
        xt2 = xt
    else:
        random_projection_dim = dim_d
        xt2 = torch.cat(
            (xt, torch.zeros((xt.shape[0], dim_d-dim_p)).to(device)), dim=1)
        xs2 = xs
