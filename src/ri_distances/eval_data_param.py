...
class DataParam():
    ...
    Args:
        metric_name (str): either 'SGW' or 'MSE'
    ...
    def generate_target(self):
        ...
        # Noise addition
        trgt_pnt_cloud += np.random.randn(*trgt_pnt_cloud.shape) * self.noise_factor
...
def evaluate_data_param(p, predictors, use_wandb=False):
    ...
    evaluate_predictors computes the difference in MSE(pred,target) and MSE(unnoised target,target) along
...
def execute_run(predictor, p, src_pnt_cloud, trgt_pnt_cloud, no_noise_target, use_wandb=False):
    ...
    min_metric = MSE(no_noise_target, trgt_pnt_cloud)
