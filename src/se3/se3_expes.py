...
expes_kwargs = [
    ({}, {"shift": 0.3}, False),  # shift
    ({"shift": 2.0, }, {"scaling": 2.0, "shift": 2.0}, False),  # shift + scale
    ({"centering": True}, {"asym": True,
                           "centering": True, "width_factor": 2.0}, False),  # sym -> asym does not work
    ({"centering": True, "asym": True},
     {"asym": True, "centering": True, "width_factor": 2.0}, False),  # asym -> asym works
    ({"asym": True, "shift": 0.3}, {
        "asym": True, "shift": 1.0}, False),  # shifted asym
    ({"shift": 2.0, }, {"scaling": 2.0, "shift": 2.0},  # asym features
     True)
]


def run(i, src_kwargs, trgt_kwargs, asym_features, quick=False, use_wandb=False):
    ...
    # Training phase
    start_training(transformer, lr, optimizer, epochs, criterion, batch_size,
                   scheduler, device, src_gen, trgt_gen, center_output, asym_features=asym_features, use_wandb=use_wandb)

    # Visualization of the prediction
    points, target_points, predicted_points = get_predictions(
        transformer, src_gen, trgt_gen, center_output)
...
def main():
    ...
    parser.add_argument(
        '-q', '--quick', help='Quick option for debugging', action='store_true')
    parser.add_argument(
        '-w', '--wandb', help='Whether to log metrics in wandb', action='store_true')
    args = parser.parse_args()

    if device.type == 'cpu':
        user_choice = input(
            "The detected device is a CPU: do you still want to execute training? (y/n)")
        if user_choice != 'y':
            return 0

    for i, (src_kwargs, trgt_kwargs, asym_features) in enumerate(tqdm(expes_kwargs, desc="SE(3) overfits experiments", leave=False)):
        run(i, src_kwargs, trgt_kwargs, asym_features, args.quick, args.wandb)
        sys.stdout.flush()
